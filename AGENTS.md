...
## Fork-specific Pi invariants

The `anhdle14` fork keeps upstream browser and annotation behavior while making Pi the workflow owner.

- Never modify or replace Pi's system prompt, AGENTS context, active tools, model, thinking level, or skills.
- Keep phase guidance in one-shot conversation messages.
- Default plans to `<cwd>/tmp/plans/<descriptive-kebab-case-slug>.md`; applicable AGENTS files and explicit user instructions may choose another Markdown path inside cwd.
- Browser review must fail closed when UI or assets are unavailable.
- Approval enters `grilling`, never implementation or external handoff.
- During grilling, investigate facts from docs and code, ask human decisions one at a time with recommendations, and require re-review if the plan changes.
- Only `plannotator_finish_grill` may unlock automatic execution or external handoff after explicit confirmation and an unchanged approved hash.
- Keep known direct file mutation tools restricted to Markdown inside cwd during planning and grilling without disabling Pi tools.
- Preserve the Shift+Tab plan-mode shortcut.
- Update from stable upstream release tags and run the full Pi-extension suite before changing the dotfiles package pin.
