...
function createHarness(cwd: string) {
	...
	return {
		...
		async submitPlan(filePath: string) {
			...
			branchEntries.splice(0, branchEntries.length, {
				...
				data: {
					phase: "grilling",
					lastSubmittedPath: filePath,
					approvedPlanHash: hashPlanContent(content),
				},
			});
			for (const handler of handlers.get("session_tree") ?? []) {
				await handler({ newLeafId: "grill", oldLeafId: null }, ctx);
			}
			return tools
				.get("plannotator_finish_grill")!
				.execute(
					"finish-grill",
					{ confirmed: true, summary: "Todos verified." },
					undefined,
					undefined,
					ctx,
				);
