...
function grillingContext(cwd: string): ReturnType<typeof createContext> {
	return createContext({
		cwd,
		entries: [{
			type: "custom",
			...
			data: {
				...
				approvalFeedback: "Verify the API boundary.",
			},
		}],
	});
}
...
describe("Plannotator phase framing messages", () => {
	test("before_agent_start never returns a systemPrompt in any phase", async () => {
		...
		const grilling = grillingContext(cwd);
		const grillingRuntime = createRuntime();
		await grillingRuntime.run("session_start", grilling);
		results.push(await startAgent(grillingRuntime, grilling));
	...
	test("grilling delivers one-shot docs/code guidance and approval notes", async () => {
		const cwd = makeWorkspace();
		writeFileSync(join(cwd, "PLAN.md"), "# Plan\n", "utf-8");
		const runtime = createRuntime();
		const context = grillingContext(cwd);
		await runtime.run("session_start", context);

		const first = await startAgent(runtime, context);
		expect(first?.message?.customType).toBe("plannotator-framing");
		expect(first?.message?.details).toEqual({ phase: "grilling" });
		expect(first?.message?.content).toContain("[PLANNOTATOR - GRILLING PHASE]");
		expect(first?.message?.content).toContain("Verify the API boundary.");
		expect(first?.message?.content).toContain("ask_user_question");
		expect(await startAgent(runtime, context)).toBeUndefined();
	});
	...
	test("executing delivers framing with an entry todo snapshot, then per-turn todo status", async () => {
		...
		expect(first?.message?.content).toContain("mandatory grill are complete");
		...
		expect(second?.message?.content).not.toContain("mandatory grill are complete");
	...
	test("null instructions disable framing but keep the todo status", async () => {
		...
		expect(first?.message?.customType).toBe("plannotator-context");
		expect(first?.message?.content).toContain("0/1 steps complete");
		expect(first?.message?.content).not.toContain("mandatory grill are complete");
	});
